...
def save_history_to_excel():
    data = {
        "Arm Counter": reps_memory.get("arm", 0),
        "Leg Counter": reps_memory.get("leg", 0),
        "Leg Counter1": reps_memory.get("leg1", 0),
    ...
    # Define the filename
    filename = "exercise_history.xlsx"
    
    # Try to append to the existing file, or create a new one if it doesn't exist
    try:
    ...
    messagebox.showinfo("History Saved", f"Exercise history has been saved to {filename}")
    ...
    # Update the GUI with the reset counters
    update_gui_reps()
        ...
        try:
            if results.pose_landmarks:
                ...
                if 'angle' in locals() and 'angle1' in locals():
                 ...
                 bar = np.interp(angle, (7, 100), (200, 650))
                 bar1 = np.interp(angle1, (7, 100), (200, 650))
    ...
    while True:
        ...
        try:
            if results.pose_landmarks:
                ...
                cv2.putText(img, 'REPSL', (50, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)
                cv2.putText(img, str(counter1), (120, 130), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 0), 2, cv2.LINE_AA)
                cv2.putText(img, 'REPSR', (700, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 2, cv2.LINE_AA)
...
def open_gui():
    ...
    history_button = Button(root, text="HISTORY", command=save_history_to_excel)
    history_button.pack()
    # Reps Section
