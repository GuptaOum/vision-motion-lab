...
def close_camera():
    ...
    global cap, exercise_running, stop_flag
    if cap is not None:
        cap.release()
...
def close_camera1():
    """Safely close the camera and release resources."""
    global cap1, exercise_running1, stop_flag
    if cap1 is not None:
        cap1.release()
    cv2.destroyAllWindows()
...
def start_arm_exercise():
    ...
    global exercise_running, cap, stop_flag
    ...
    # Rest of the arm exercise code remains the same...
...
def start_leg_exercise():
    ...
    global exercise_running1, cap1, stop_flag
...
def open_gui():
    ...
    def exit_gui():
        global stop_flag
        stop_flag = True
        # Ensure any open camera windows are closed
        if cap is not None:
            close_camera()
        if cap1 is not None:
            close_camera1()
    ...
    arm_button = Button(
        ...
        text="Arm Exercise",
        command=lambda: [
            select_button(arm_button), 
            Thread(target=start_arm_exercise).start()
        ],
        width=20,
        font=("Arial", 12), 
    )
    ...
    leg_button = Button(
        ...
        text="Leg Exercise",
        command=lambda: [
            select_button(leg_button), 
            Thread(target=start_leg_exercise).start()
        ],
        width=20,
         font=("Arial", 12),
    )
